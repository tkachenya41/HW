...
function logPerson(person: Person, index: number) {
  ...
  const registeredAt = dateWizard(
    person.registered,
    '{date}.{month}.{year} {hours}:{minutes}'
  );
  const number_ = `#${dateWizard.pad(index + 1)}`;
  return `- ${number_}: ${person.name}, ${person.age}, ${additionalInformation}, ${registeredAt}`;
}
...
console.debug('Early birds:');

console.debug(
  [...([] as Person[]), ...users, ...admins]
...
export * as dateWizard from 'date-wizard';
